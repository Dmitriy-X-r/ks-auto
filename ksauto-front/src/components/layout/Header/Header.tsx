...
import {HeaderActions} from "@/components/layout/Header/HeaderAction";
import {getAuthToken} from "@/lib/auth/getAuthToken";
import {HeaderProfileTitle} from "@/components/utils/profile_menu/HeaderProfileTitle";
...
export default function Header({ catalogsData }: HeaderProps) {
    ...
    const [profileData, setProfileData] = useState<ProfileMenuApiResponse | null>(null);

    useEffect(() => {
        const token = getAuthToken();
        const auth = !!token && token !== "not_found";
        setIsAuth(auth);
        ...
        if (auth) {
            ...
            fetch("/api/profile_user_menu", {
                ...
                .then(res => res.json())
                .then(res => setProfileData(res.item ?? null))
        ...
        <>
                    ...
                    <div className="new-container">
                        ...
                        <div className={`header-burger-wrapper${isBurgerOpen ? " show" : ""}${isBurgerAccountMenu ? " menu-show" : ""}`}>
                            <div className="header-burger-top">
                                {/* not auth */}
                                {isAuth ? (
                                    <HeaderProfileTitle data={profileData} />
                                ) : (
                                    <Link
                                        href="/auth/"
                                        className="header-account header-menu__top-auth"
                        ...
                        <HeaderActions isAuth={isAuth} profileData={profileData}/>
            ...
            <div className={`popup-add-offer${isAddOfferOpen ? " show" : ""}`}>
                ...
                <div className="popup-add-offer-body">
                    ...
                    <p className="popup-add-offer-title">Выберите тип транспорта</p>
